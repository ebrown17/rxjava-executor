...
public class IdGenerator {
    ...
    /**
     * IdGenerator generates unique id's. Id's are kept in an Integer pool. The pool has an initial size
     * of 1000 and when this is reached; it increments by 1000. This happens until the max pool size is
     * reached which by default 50K. Both the pool increment size and the max size can be overridden.
     *
     ...
     *                    mean use default of 1000. If this is set, it must be <= half of maxSize and maxSize
     *                    must also not be 0; or default values will be used.
     * @param maxSize     of the id pool for this generator. Setting <= 0 will mean use default of 50K.
     *                    If set, it must be >= 2 times poolIncSize and poolIncSize must not be 0; or default values
     *                    will be used.
     */
